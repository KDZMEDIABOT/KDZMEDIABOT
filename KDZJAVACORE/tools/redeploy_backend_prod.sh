...
DOCKER_BUILDKIT=1 docker compose $OPT_LOG -f "${COMPOSE_FILE}" --env-file "${PROD_ENV_FILE}" up -d --build redis postgres backend auth_sidecar readingplus_mcp_sidecar
DOCKER_BUILDKIT=1 docker compose $OPT_LOG -f "${COMPOSE_FILE}" --env-file "${PROD_ENV_FILE}" restart redis backend readingplus_mcp_sidecar
...
DOCKER_BUILDKIT=1 docker compose $OPT_LOG -f "${COMPOSE_FILE}" --env-file "${PROD_ENV_FILE}" logs --tail=200 backend auth_sidecar readingplus_mcp_sidecar
exit 1
